...
export default function StatisticsCards({ reports }: StatisticsCardsProps) {
  ...
  const positiveReports = reports.filter(r => r.sentiment === 'positive').length;
  const negativeReports = reports.filter(r => r.sentiment === 'negative').length;
  
  const positivePercentage = totalReports > 0 ? (positiveReports / totalReports) * 100 : 0;
  const negativePercentage = totalReports > 0 ? (negativeReports / totalReports) * 100 : 0;
