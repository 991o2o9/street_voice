...
function App() {
  ...
  const categories = Array.from(new Set(reports.filter(r => r.category).map(r => r.category!)));
  ...
  const handleDataLoaded = (newReports: Report[]) => {
    setReports(prevReports => {
      // Объединяем новые данные с существующими, избегая дубликатов
    ...
    try {
      ...
      setReports(prevReports => 
        prevReports.map(report => {
          const index = unanalyzedReports.findIndex(ur => ur.id === report.id);
          if (index !== -1) {
